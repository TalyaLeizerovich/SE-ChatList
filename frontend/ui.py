...
def send_message(content, date, time, sender_name, group_name):
    payload = {
        "content": content,
        "date": date,
        "time": time,
...
with gr.Blocks(title="ChatList UI") as demo:
    gr.Markdown("## ChatList – Frontend (Gradio)")

    content = gr.Textbox(label="Message Content")
    date = gr.Textbox(label="Date (YYYY-MM-DD)")
    time = gr.Textbox(label="Time (HH:MM)")
    sender = gr.Textbox(label="Sender Name")
    group = gr.Textbox(label="Group Name")

    output = gr.Textbox(label="Backend Response", lines=10)

    submit_btn = gr.Button("Send to Backend")
    submit_btn.click(send_message, [content, date, time, sender, group], output)
